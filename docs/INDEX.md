# TruePay documentation index

## Overview

| Doc | Audience |
|-----|----------|
| [`README_HIGH_LEVEL.md`](./README_HIGH_LEVEL.md) | Architecture summary |
| [`BACKEND_ARCHITECTURE.md`](./BACKEND_ARCHITECTURE.md) | Services, ledger, data model |
| [`readme.md`](./readme.md) | Function reference & troubleshooting |
| [`overview.md`](./overview.md) | Product overview |

## Consumer (C2B)

| Doc | Topic |
|-----|-------|
| [`api.md`](./api.md) | REST + callables |
| [`rates.md`](./rates.md) | P2P / customer rates, Send→Get quotes, cross pairs |
| [`auth.md`](./auth.md) | Authentication |
| [`FRONTEND_AUTH_HANDOFF.md`](./FRONTEND_AUTH_HANDOFF.md) | Claims, routing, token refresh |
| [`PAYSTACK_TOURIST.md`](./PAYSTACK_TOURIST.md) | Paystack wallet top-up |
| [`TRANSACTIONS_API.md`](./TRANSACTIONS_API.md) | Transaction history API |
| [`circle_c2b.md`](./circle_c2b.md) | Crypto (Circle) Flutter guide |
| [`CIRCLE.md`](./CIRCLE.md) | Crypto backend |
| [`pay.md`](./pay.md) | Safari Card pay / payout (Flutter) |
| [`C2B_PAYLOAD_ENCRYPTION.md`](./C2B_PAYLOAD_ENCRYPTION.md) | Optional C2B mobile payload encryption |
| [`SAFARI_CARD_PAYOUTS.md`](./SAFARI_CARD_PAYOUTS.md) | Safari Card backend |
| [`PAYMENTS_CLIENT_FIXES.md`](./PAYMENTS_CLIENT_FIXES.md) | Client integration notes |

## B2B

| Doc | Topic |
|-----|-------|
| [`B2B_docs.md`](./B2B_docs.md) | Partner & portal API reference |
| [`onboarding.md`](./onboarding.md) | Partner onboarding steps |
| [`B2B_FRONTEND_INSTRUCTIONS.md`](./B2B_FRONTEND_INSTRUCTIONS.md) | Dashboard UI |
| [`B2B_SANDBOX.md`](./B2B_SANDBOX.md) | Public sandbox API |
| [`B2B_SANDBOX_DASHBOARD_FRONTEND.md`](./B2B_SANDBOX_DASHBOARD_FRONTEND.md) | Sandbox dashboard |
| [`B2B_ADD_MONEY.md`](./B2B_ADD_MONEY.md) | Partner Add Money |
| [`B2B_SEND.md`](./B2B_SEND.md) | B2B Send |
| [`PRODUCT_PRICING.md`](./PRODUCT_PRICING.md) | Revenue Calculator product fees |
| [`aed.md`](./aed.md) | AED Send: what is implemented |
| [`B2B_ADMIN_WALLET.md`](./B2B_ADMIN_WALLET.md) | Admin wallet |
| [`B2B_CREATE_PARTNER.md`](./B2B_CREATE_PARTNER.md) | Create partner |

## Ops & payments

| Doc | Topic |
|-----|-------|
| [`PAYMENT_LIFECYCLE.md`](./PAYMENT_LIFECYCLE.md) | Funding + settlement flows |
| [`FUNDING_OPS.md`](./FUNDING_OPS.md) | Runbooks & jobs |
| [`INTASEND.md`](./INTASEND.md) | IntaSend collection (B2B links) |
| [`TRANSAK_TOURIST.md`](./TRANSAK_TOURIST.md) | Transak funding |

## Other

| Doc | Topic |
|-----|-------|
| [`postman.md`](./postman.md) | Postman collections |
| [`run.md`](./run.md) | Run notes |
| [`REFACTORING_SUMMARY.md`](./REFACTORING_SUMMARY.md) | Refactoring notes |
| [`answer.md`](./answer.md) / [`answer2.md`](./answer2.md) | Design Q&A |

Repo conventions: [`../AGENTS.md`](../AGENTS.md)
